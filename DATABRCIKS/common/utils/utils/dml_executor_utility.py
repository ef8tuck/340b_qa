...
#   Output              :   
#   Pre-requisites      :   
#   Last changed on     :   10 Aug, 2024
#   Last changed by     :   Raghav Haralaka / Vijit Vijayvargiya
#   Reason for change   :   Added capabilitiy to work with multiple Catalog.
# ######################################################################################################################
...
try:
    ...
    catalog_name = dbutils.widgets.get('catalog_name') #changes
    dict_catalog = dbutils.widgets.get('dict_catalog') #changes
    options['catalog_name'] = catalog_name
    for key, val in eval(dict_catalog).items(): #changes
        options[key] = val
